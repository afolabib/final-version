...
import {
  ...
  Zap, Sparkles, X, ArrowRight, Clock, Flag, Bell
} from 'lucide-react';
...
export default function FreemiCommandCenter() {
  const { company, agents, goals, tasks, pendingApprovals, activeCompanyId, ceoAgent } = useCompany();
  const { user } = useAuth();
  ...
  const [quickLook, setQuickLook] = useState(null); // 'tasks' | 'goals' | null
  ...
  const handleSubmit = async () => {
    ...
    navigate(`/dashboard/chat?q=${encodeURIComponent(text)}`);
  };
        ...
        {activeAgents.length > 0 ? (
          <div className="flex items-center gap-2 overflow-x-auto" style={{ scrollbarWidth: 'none' }}>
            {activeAgents.map((agent, i) => {
              ...
              const colors = ['#5B5FFF','#00B894','#0984E3','#E17055','#FDCB6E','#A29BFE','#74B9FF'];
              const color = colors[i % colors.length];
              const isActive = agent.status === 'active';
              return (
                <div key={agent.id} className="flex items-center gap-1.5 px-2.5 py-1 rounded-full flex-shrink-0"
                  style={{
                    background: `${color}0D`,
                  ...
                  <div className="relative flex-shrink-0">
                    ...
                    <div className="absolute -bottom-0.5 -right-0.5 w-2 h-2 rounded-full border border-white"
                      style={{ background: isActive ? '#10B981' : '#FDCB6E' }} />
                  </div>
                  <span className="text-[11px] font-semibold" style={{ color }}>{agent.name}</span>
                </div>
              );
            })}
          </div>
      ...
      <div className="flex-1 flex flex-col items-center justify-center px-6 pb-6 overflow-y-auto">
        ...
        <motion.div
          ...
          <div className="absolute inset-0 scale-[1.4] rounded-full"
            style={{ background: 'radial-gradient(circle, rgba(91,95,255,0.12) 0%, transparent 70%)', filter: 'blur(12px)' }} />
