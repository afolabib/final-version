...
const channels = [
  ...
  { icon: Users, title: 'Community Slack', sub: 'Join 5000+ users', note: 'Community driven', color: '#8B5CF6' },
    ...
    <div className="flex-1 overflow-y-auto px-4 md:px-8 py-6 md:py-10">
      ...
      <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }} className="mb-10">
        <div className="inline-block mb-4">
          <span className="text-[10px] font-bold tracking-[0.2em] uppercase px-4 py-1.5 rounded-full" style={{ background: 'rgba(74,108,247,0.08)', color: '#4A6CF7', border: '1px solid rgba(74,108,247,0.15)' }}>Support Center</span>
        </div>
      ...
      <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3, delay: 0.05 }}
        ...
        <Search size={16} style={{ color: '#4A6CF7' }} />
      ...
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.3, delay: 0.08 }}
        ...
        {helpTabs.map(t => (
          <motion.button key={t.id} onClick={() => setTab(t.id)}
            ...
            style={{
              background: tab === t.id ? 'linear-gradient(135deg, #4A6CF7, #6366F1)' : '#FFFFFF',
      ...
      <div className="grid grid-cols-1 gap-6 mb-14">
        {currentFaqs.map((faq, i) => (
          <motion.div key={i}
            ...
            <div className="absolute -top-6 -right-6 w-20 h-20 rounded-full blur-2xl opacity-0 group-hover:opacity-15 transition-opacity pointer-events-none" style={{ background: '#4A6CF7' }} />
              ...
              <motion.div key={i}
                ...
                {c.badge && (
                  <span className="text-[11px] font-bold px-3 py-1 rounded-full mb-2 transition-all"
                    style={{ background: 'linear-gradient(135deg, rgba(74,108,247,0.12), rgba(99,102,241,0.08))', color: '#4A6CF7', border: '1px solid rgba(74,108,247,0.2)' }}>{c.badge}</span>
                )}
      ...
      <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4, delay: 0.35 }}
        ...
        <div className="absolute -top-10 -right-10 w-32 h-32 rounded-full blur-3xl opacity-0 group-hover:opacity-10 transition-opacity pointer-events-none" style={{ background: '#4A6CF7' }} />
        ...
        <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }}
          ...
          style={{ background: 'linear-gradient(135deg, #4A6CF7, #6366F1)', boxShadow: '0 8px 24px rgba(74,108,247,0.3)' }}>
          Send message
        </motion.button>
      </motion.div>
      ...
      <p className="text-xs text-center" style={{ color: '#9CA3AF' }}>
        ...
        <span className="cursor-pointer hover:text-[#4A6CF7] transition-colors" style={{ color: '#9CA3AF' }}>Privacy Policy</span> &nbsp;·&nbsp;
        <span className="cursor-pointer hover:text-[#4A6CF7] transition-colors" style={{ color: '#9CA3AF' }}>Terms of Service</span>
      </p>
      ...
      {showChat && (
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="fixed bottom-6 right-6 w-96 rounded-2xl shadow-2xl z-50" style={{ background: '#FFFFFF', border: '1px solid #E8EAFF' }}>
          {/* Header */}
          <div className="px-6 py-4 flex items-center justify-between" style={{ borderBottom: '1px solid #E8EAFF', background: 'linear-gradient(135deg, #4A6CF7, #6366F1)' }}>
            <div>
          ...
          <div className="px-4 py-3 flex gap-2" style={{ borderTop: '1px solid #E8EAFF' }}>
            ...
            />
            <button onClick={handleSendMessage} className="p-2 rounded-lg transition-all text-white" style={{ background: '#4A6CF7' }}>
              <Send size={16} />
            </button>
          </div>
        </motion.div>
