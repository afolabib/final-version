    ...
    >
      ...
      {menuOpen && (
        <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }}
          ...
          <Link to="/about" className="block w-full text-left px-3 py-2.5 text-sm text-gray-600 hover:text-surface font-medium rounded-lg hover:bg-gray-50" onClick={() => setMenuOpen(false)}>Product</Link>
          <Link to="/solutions" className="block w-full text-left px-3 py-2.5 text-sm text-gray-600 hover:text-surface font-medium rounded-lg hover:bg-gray-50" onClick={() => setMenuOpen(false)}>Solutions</Link>
          ...
          <div className="pt-3 flex flex-col gap-2">
            <button onClick={() => navigate('/dashboard')} className="w-full px-4 py-2.5 text-sm font-semibold text-white rounded-full"
              style={{ background: 'linear-gradient(135deg, #6C5CE7, #7C6CF7)' }}>Start Free Trial →</button>
          </div>
        </motion.div>
      )}
    </motion.header>
