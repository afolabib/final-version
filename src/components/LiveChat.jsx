...
export default function LiveChat({ onClose }) {
  ...
  const handleSend = async () => {
    ...
    try {
      const replies = [
        "Freemi lets you give goals to an AI CEO that breaks them down into tasks and delegates to specialized agents — sales, engineering, support, and more.",
        ...
        "Freemi is built on a Paperclip-style orchestration model: CEO agent at the top, specialized sub-agents handling each function.",
      ];
  ...
  return (
    <div className="flex flex-col h-full md:rounded-2xl overflow-hidden"
      style={{
        ...
        boxShadow: '0 25px 60px rgba(91,95,255,0.15), 0 8px 20px rgba(0,0,0,0.06)',
      ...
      <div className="relative px-5 py-4 flex items-center gap-3"
        style={{
          background: 'linear-gradient(135deg, #5B5FFF 0%, #7C6CF7 50%, #8B7CF8 100%)',
        }}>
              ...
              >
                {!isUser && (
                  <div className="w-7 h-7 rounded-lg flex-shrink-0 flex items-center justify-center mt-0.5"
                    style={{ background: 'linear-gradient(135deg, #EDE9FF, #F3F0FF)', border: '1px solid rgba(91,95,255,0.08)' }}>
                    <div className="w-2 h-2 rounded-full" style={{ background: '#5B5FFF' }} />
                  </div>
                )}
                <div
                  ...
                  style={isUser
                    ? { background: 'linear-gradient(135deg, #5B5FFF, #7C6CF7)', boxShadow: '0 2px 8px rgba(91,95,255,0.25)' }
      ...
      {/* Input */}
      <div className="px-4 py-3 border-t" style={{ borderColor: 'rgba(91,95,255,0.06)', background: 'rgba(248,247,255,0.5)' }}>
        <div className="flex gap-2 items-end">
          ...
          <motion.button
            ...
            style={{
              background: input.trim() && !loading ? 'linear-gradient(135deg, #5B5FFF, #7C6CF7)' : 'rgba(91,95,255,0.08)',
              boxShadow: input.trim() && !loading ? '0 3px 12px rgba(91,95,255,0.3)' : 'none',
