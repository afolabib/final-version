...
import {
  HeartPulse, Calendar, FileText, Shield, Phone, Activity, Zap, ArrowRight, Check,
  Star, Bot, Sparkles, Globe, Bell, Users, Stethoscope, Brain, Pill, ClipboardList, X
} from 'lucide-react';
import TopNav from '../../components/TopNav';
import SiteFooter from '../../components/SiteFooter';
import ScrollReveal from '../../components/ScrollReveal';

const ORB_COLORS = ['rgba(232,67,147,0.22)', 'rgba(123,97,255,0.16)', 'rgba(47,143,255,0.14)', 'rgba(39,192,135,0.12)', 'rgba(168,85,247,0.18)'];
const FLOAT_ICONS = [Sparkles, Bot, Zap, HeartPulse, Star, Stethoscope, Brain, Shield];
...
function FloatingBg({ seed = 42 }) {
  ...
  return (
    <div className="absolute inset-0 overflow-hidden pointer-events-none">
      ...
      {gen(10).map((p, i) => { const Icon = FLOAT_ICONS[i % FLOAT_ICONS.length]; return <motion.div key={`i${i}`} className="absolute text-pink-500/[0.12]" style={{ left: p.left, top: p.top }} animate={{ y: [0, -15, 0], rotate: [0, 8, -8, 0], scale: [1, 1.1, 1] }} transition={{ duration: p.duration, repeat: Infinity, ease: 'easeInOut', delay: p.delay }}><Icon style={{ width: p.size, height: p.size }} /></motion.div>; })}
    </div>
  );
}
...
const features = [
  { icon: Calendar, title: 'Smart appointment management', desc: 'Automated confirmations, reminders via WhatsApp/SMS, waitlist notifications, and easy rescheduling. No-shows drop by 40%.', color: '#E84393', span: 'md:col-span-2 md:row-span-2' },
  ...
  { icon: Activity, title: 'Patient insights', desc: 'Track appointment volume, no-show rates, common enquiries, and patient satisfaction. Data-driven practice management.', color: '#0984E3', span: 'md:col-span-2' },
];

const differentiators = [
  { label: 'AI appointment scheduling', us: true, others: false },
  { label: 'WhatsApp & SMS reminders', us: true, others: false },
  { label: 'Automated patient follow-ups', us: true, others: false },
  { label: 'Billing query handling', us: true, others: false },
  { label: 'Phone answering & triage', us: true, others: false },
  { label: 'Compliance documentation', us: true, others: false },
  ...
  return (
    <div className="relative w-full text-surface" style={{ background: 'linear-gradient(180deg, #FFF0F6 0%, #F8F9FE 30%, #FFF5F8 60%, #F8F9FE 100%)', minHeight: '100vh', overflowX: 'clip' }}>
      ...
      <section className="relative overflow-hidden pt-28 md:pt-36 pb-16">
        ...
        <div className="absolute inset-0 pointer-events-none opacity-40" style={{ backgroundImage: 'radial-gradient(rgba(232,67,147,0.06) 1px, transparent 1px)', backgroundSize: '32px 32px' }} />
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-6">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
            ...
            <motion.div variants={containerV} initial="hidden" animate="visible">
              <motion.div variants={itemV}>
                <div className="inline-flex items-center gap-2.5 px-4 py-2 rounded-full bg-white/70 backdrop-blur-xl border border-pink-200/30 shadow-sm mb-8">
                  <HeartPulse className="w-3.5 h-3.5" style={{ color: AC }} />
                  <span className="text-xs font-semibold text-surface/80 tracking-wide">Freemi | Healthcare</span>
                </div>
              </motion.div>
              <motion.h1 variants={itemV} className="text-5xl sm:text-6xl lg:text-7xl font-extrabold tracking-tight leading-[1.1] mb-6">
                Let Your Staff Focus<br />
                <span style={{ color: AC }}>On Patients, Not Paperwork.</span>
              </motion.h1>
              <motion.p variants={itemV} className="text-base md:text-lg text-gray-500 mb-8 leading-relaxed max-w-lg">
                AI agents that automate appointment scheduling, patient follow-ups, billing enquiries, and compliance documentation — so your clinical team delivers better care.
              </motion.p>
              <motion.div variants={itemV} className="flex flex-col sm:flex-row gap-3 mb-8">
                <motion.div whileHover={{ scale: 1.03 }} whileTap={{ scale: 0.97 }}>
                  <button onClick={() => navigate('/contact')}
                    ...
                    style={{ background: `linear-gradient(135deg, ${AC}, #C2185B)`, boxShadow: `0 8px 32px rgba(232,67,147,0.35)` }}>
            ...
            <motion.div className="hidden lg:block relative" style={{ perspective: '1200px' }}
              ...
              <div className="relative rounded-2xl overflow-hidden shadow-2xl" style={{ background: 'rgba(255,255,255,0.95)', border: '1px solid rgba(0,0,0,0.06)', boxShadow: `0 50px 100px rgba(232,67,147,0.2), 0 0 80px rgba(232,67,147,0.08)`, transform: 'rotateY(-3deg) rotateX(2deg)' }}>
                {/* chrome */}
                ...
                <div className="p-5">
                  <div className="flex items-center justify-between mb-4">
                    <div className="flex items-center gap-2"><HeartPulse className="w-4 h-4 text-pink-500" /><span className="text-[11px] font-bold text-gray-800">Patient Dashboard</span></div>
                    <span className="text-[10px] px-2 py-0.5 rounded-full bg-emerald-50 text-emerald-600 font-bold">AI Active</span>
                  </div>
                  {/* stats row */}
                  <div className="grid grid-cols-4 gap-2 mb-4">
                    {[{ label: 'Appointments Today', val: '24', color: '#E84393' }, { label: 'No-Shows', val: '6%', color: '#27C087' }, { label: 'AI Handled', val: '87%', color: '#7B61FF' }, { label: 'Reminders', val: '156', color: '#2F8FFF' }].map(s => (
                  ...
                  <div className="space-y-1.5">
                    {[
                      { time: '09:00', patient: "Mary O'Brien", type: 'Check-up', status: 'Confirmed', sc: 'bg-emerald-50 text-emerald-600' },
                      { time: '09:45', patient: 'James Kelly', type: 'Follow-up', status: 'Confirmed', sc: 'bg-emerald-50 text-emerald-600' },
                      { time: '10:30', patient: 'Priya Patel', type: 'Consultation', status: 'Reminder Sent', sc: 'bg-blue-50 text-blue-600' },
                      { time: '11:15', patient: 'Patrick Murphy', type: 'Prescription Review', status: 'Pending', sc: 'bg-amber-50 text-amber-600' },
                      { time: '14:00', patient: 'Sarah Chen', type: 'Dental Clean', status: 'Confirmed', sc: 'bg-emerald-50 text-emerald-600' },
                    ].map(a => (
                      <div key={a.time} className="flex items-center gap-2.5 px-2.5 py-2 rounded-lg border border-gray-50">
                        ...
                        <div className="w-6 h-6 rounded-full bg-pink-50 flex items-center justify-center shrink-0"><span className="text-[8px] font-bold text-pink-600">{a.patient.split(' ').map(n => n[0]).join('')}</span></div>
              ...
              <motion.div animate={{ y: [0, -8, 0] }} transition={{ duration: 4, repeat: Infinity, ease: 'easeInOut' }}
                ...
                <div className="w-6 h-6 rounded-full bg-pink-500/15 flex items-center justify-center"><HeartPulse className="w-3 h-3 text-pink-600" /></div>
                <span className="text-xs font-semibold text-surface">40% fewer no-shows</span>
              </motion.div>
              <motion.div animate={{ y: [0, -6, 0] }} transition={{ duration: 5, repeat: Infinity, ease: 'easeInOut', delay: 1.5 }}
                ...
                <span className="text-xs font-semibold text-surface">87% AI-handled</span>
              </motion.div>
            </motion.div>
          </div>
        </div>
      ...
      <section className="py-12 px-6">
        <div className="max-w-5xl mx-auto grid grid-cols-2 md:grid-cols-4 gap-6">
          {[{ v: '40%', l: 'Fewer no-shows' }, { v: '87%', l: 'AI-handled enquiries' }, { v: '8hrs', l: 'Admin saved weekly' }, { v: '24/7', l: 'Patient support' }].map((s, i) => (
      ...
      {/* ═══ 3-STEP PROCESS ═══ */}
      <section className="py-20 px-6 relative overflow-hidden" style={{ background: `linear-gradient(180deg, rgba(232,67,147,0.03), rgba(123,97,255,0.02))` }}>
        <FloatingBg seed={184} />
        <div className="relative z-10 max-w-5xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-10 relative">
            ...
            {[
              { step: '01', icon: Stethoscope, title: 'Tell us about your practice', desc: 'Share your services, scheduling rules, and patient communication preferences. We configure everything.', color: AC, ring: 'rgba(232,67,147,0.1)', lightBg: 'rgba(232,67,147,0.06)' },
              { step: '02', icon: Brain, title: 'AI agents go live', desc: 'Custom-trained agents handle appointments, follow-ups, billing, and patient messages across all channels.', color: '#7B61FF', ring: 'rgba(123,97,255,0.1)', lightBg: 'rgba(123,97,255,0.06)' },
              { step: '03', icon: Zap, title: 'Your practice runs smoother', desc: 'Staff focus on care, not admin. Patients get instant responses. No-shows plummet.', color: '#2F8FFF', ring: 'rgba(47,143,255,0.1)', lightBg: 'rgba(47,143,255,0.06)' },
            ].map((p, i) => (
      ...
      <section className="py-20 px-6 relative overflow-hidden">
        ...
        <div className="relative z-10 max-w-3xl mx-auto">
          <div className="text-center mb-12">
            <ScrollReveal><h2 className="text-4xl md:text-5xl font-extrabold tracking-tight text-surface">Freemi vs <span style={{ color: AC }}>Typical Clinic.</span></h2></ScrollReveal>
          ...
          <ScrollReveal delay={0.15}>
            <div className="rounded-2xl overflow-hidden" style={{ background: 'rgba(255,255,255,0.92)', backdropFilter: 'blur(24px)', border: '1px solid rgba(0,0,0,0.06)', boxShadow: `0 16px 48px rgba(232,67,147,0.06)` }}>
              <div className="grid grid-cols-3 text-xs font-bold uppercase tracking-wider px-6 py-4" style={{ background: `linear-gradient(135deg, rgba(232,67,147,0.05), rgba(123,97,255,0.03))` }}>
                <span className="text-gray-400">Capability</span>
                <span className="text-center text-gray-400">Typical Clinic</span>
                <span className="text-center" style={{ color: AC }}>Freemi Healthcare</span>
              </div>
      ...
      {/* ═══ USE CASES ═══ */}
      <section className="py-20 px-6 relative overflow-hidden" style={{ background: `linear-gradient(180deg, rgba(232,67,147,0.04), rgba(123,97,255,0.02))` }}>
        <FloatingBg seed={188} />
        <div className="relative z-10 max-w-5xl mx-auto">
          <div className="text-center mb-12">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
            {[
              { icon: Stethoscope, title: 'General Practice', desc: 'Appointment booking, repeat prescriptions, test results notifications, and health check reminders.', color: '#E84393', metric: '87%', metricLabel: 'AI-handled' },
              { icon: HeartPulse, title: 'Dental Clinics', desc: 'Cleaning reminders, check-up scheduling, treatment plan follow-ups, and insurance queries.', color: '#7B61FF', metric: '40% fewer', metricLabel: 'No-shows' },
              { icon: Activity, title: 'Physiotherapy', desc: 'Session bookings, exercise plan reminders, progress check-ins, and referral management.', color: '#2F8FFF', metric: '3x', metricLabel: 'Efficiency' },
              { icon: Pill, title: 'Pharmacy', desc: 'Prescription refills, medication reminders, delivery scheduling, and drug interaction queries.', color: '#27C087', metric: '92%', metricLabel: 'Auto-resolved' },
              { icon: Users, title: 'Mental Health', desc: 'Session scheduling, mood check-ins, resource sharing, and crisis protocol escalation.', color: '#F59E0B', metric: '24/7', metricLabel: 'Support' },
              { icon: ClipboardList, title: 'Specialist Clinics', desc: 'Referral processing, pre-appointment preparation, post-procedure follow-ups, and results.', color: '#0984E3', metric: '8hrs', metricLabel: 'Saved weekly' },
            ].map((uc, i) => (
      ...
      <section className="py-20 px-6 relative overflow-hidden">
        ...
        <div className="relative z-10 max-w-5xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
            {[
              { quote: 'Our no-show rate dropped from 22% to 6% in the first month. The reminder sequences alone paid for the platform.', name: 'Dr. James Park', role: 'Practice Director, Meridian Health', gradient: `linear-gradient(135deg, ${AC}, #7B61FF)` },
      ...
      <section className="py-16 px-6">
        <ScrollReveal direction="none">
          <motion.div className="max-w-4xl mx-auto rounded-3xl relative overflow-hidden py-16 px-8 text-center"
            style={{ background: `linear-gradient(135deg, ${AC}, #C2185B, #7B61FF)`, boxShadow: `0 32px 80px rgba(232,67,147,0.3)` }}>
            {[Sparkles, HeartPulse, Stethoscope, Brain, Zap, Bot, Star].map((Icon, i) => (
            ...
            <div className="relative z-10">
              ...
              <p className="mt-4 text-base text-white/70 max-w-md mx-auto">AI agents that handle the paperwork so your team can focus on patients.</p>
