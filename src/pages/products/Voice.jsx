  ...
  return (
    <div className="relative w-full text-surface" style={{ background: 'linear-gradient(180deg, #F5F3FF 0%, #F8F9FE 30%, #EEF2FF 60%, #F0F7FF 100%)', minHeight: '100vh', overflowX: 'clip' }}>
      ...
      <section className="py-20 px-6">
        <div className="text-center mb-12"><ScrollReveal><h2 className="text-4xl md:text-5xl font-extrabold tracking-tight text-surface">Simple <span style={{ color: '#27C087' }}>pricing.</span></h2></ScrollReveal></div>
            ...
            <div className="rounded-2xl p-7 text-left relative" style={{ background: 'rgba(255,255,255,0.95)', border: `2px solid #27C08730`, boxShadow: `0 16px 48px #27C08712`, transform: 'scale(1.02)' }}>
              ...
              <div className="space-y-2.5">
                {['Everything in Starter', 'Appointment booking', 'Order taking', 'Smart routing', 'CRM integration', 'Priority support'].map(f => (
                  <div key={f} className="flex items-center gap-2.5"><div className="w-4 h-4 rounded-full flex items-center justify-center shrink-0" style={{ background: '#27C08715' }}><Check className="w-2.5 h-2.5" style={{ color: '#27C087' }} /></div><span className="text-xs text-gray-600">{f}</span></div>
                ))}
              </div>
              <motion.div whileHover={{ scale: 1.03 }} whileTap={{ scale: 0.97 }} className="mt-6">
                <button onClick={() => window.open('https://studio.freemi.ai/signup', '_blank')} className="w-full py-3.5 rounded-full text-white font-bold text-sm" style={{ background: `linear-gradient(135deg, #27C087, #27C087CC)`, boxShadow: `0 6px 20px #27C08740` }}>Get started →</button>
              </motion.div>
            </div>
            {/* Scale */}
            <div className="rounded-2xl p-7 text-left" style={{ background: 'rgba(255,255,255,0.85)', border: '1px solid rgba(0,0,0,0.06)' }}>
