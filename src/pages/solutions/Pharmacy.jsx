...
import {
  ...
  ArrowRight, Check, Sparkles, Zap, Shield, Clock, Send, Star,
  Truck, Heart, ShoppingCart, Package, X
} from 'lucide-react';
...
const FLOAT_ICONS = [Sparkles, Bot, Zap, Pill, Star, Phone, Globe, Shield];
...
const features = [
  { icon: Clock, title: 'AI Phone Assistant', desc: 'Automated confirmations, reminders via WhatsApp/SMS, waitlist notifications, and easy rescheduling. No-shows drop by 89%.', color: '#E84393', span: 'md:col-span-2 md:row-span-2' },
  { icon: Send, title: 'WhatsApp Ordering', desc: 'Patients order in the app they already use. Photo prescriptions, repeat requests, delivery scheduling — all via chat.', color: '#7B61FF' },
  { icon: Truck, title: 'Unified Order Queue', desc: 'All orders from every channel — phone, web, WhatsApp, walk-in — in one view. Prioritise, process, dispatch.', color: '#2F8FFF' },
  { icon: Shield, title: 'Delivery Management', desc: 'Maintains required documentation -- from audit trails to regulatory logs -- keeping your practice compliant without manual work.', color: '#27C087' },
  { icon: Phone, title: 'Automated Reminders', desc: 'Prescription refill reminders, collection notifications, and delivery updates sent automatically via WhatsApp.', color: '#F59E0B' },
  { icon: BarChart3, title: 'Patient Profiles', desc: 'Complete history across every interaction. Medications, preferences, delivery addresses, communication history.', color: '#0984E3', span: 'md:col-span-2' },
];

const differentiators = [
  { label: 'AI appointment scheduling', us: true, others: false },
  { label: 'WhatsApp & SMS reminders', us: true, others: false },
  { label: 'WhatsApp Ordering', us: true, others: false },
  { label: 'Billing query handling', us: true, others: false },
  { label: 'Phone answering & triage', us: true, others: false },
  { label: 'Delivery Management', us: true, others: false },
    ...
    <div className="relative w-full text-surface" style={{ background: 'linear-gradient(180deg, #FFF0F6 0%, #F8F9FE 30%, #FFF5F8 60%, #F8F9FE 100%)', minHeight: '100vh', overflowX: 'clip' }}>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
            ...
            <motion.div className="hidden lg:block relative" style={{ perspective: '1200px' }}
              ...
              <div className="relative rounded-2xl overflow-hidden shadow-2xl" style={{ background: 'rgba(255,255,255,0.95)', border: '1px solid rgba(0,0,0,0.06)', boxShadow: `0 50px 100px rgba(123,97,255,0.2), 0 0 80px rgba(123,97,255,0.08)`, transform: 'rotateY(-3deg) rotateX(2deg)' }}>
                ...
                <div className="p-5">
                  ...
                  <div className="grid grid-cols-4 gap-2 mb-4">
                    {[{ label: 'Orders Today', val: '47', color: '#E84393' }, { label: 'No-Shows', val: '89%', color: '#27C087' }, { label: 'AI Handled', val: '60%', color: '#7B61FF' }, { label: 'Reminders', val: '18', color: '#2F8FFF' }].map(s => (
                  ...
                  {/* appointment list */}
                  <p className="text-[9px] font-bold text-gray-400 uppercase tracking-wider mb-2">Orders Queue</p>
                  <div className="space-y-1.5">
                    {[
                      { time: '09:00', patient: "Mary O'Brien", type: 'Vitamin D3, Omega-3', status: 'Confirmed', sc: 'bg-emerald-50 text-emerald-600' },
                      { time: '09:45', patient: 'Patrick Kelly', type: 'Metformin (repeat)', status: 'Confirmed', sc: 'bg-emerald-50 text-emerald-600' },
                      { time: '10:30', patient: 'Siobhan Walsh', type: 'Flu Vaccine booking', status: 'Reminder Sent', sc: 'bg-blue-50 text-blue-600' },
                      { time: '11:15', patient: 'Declan Murphy', type: 'Salbutamol inhaler', status: 'Pending', sc: 'bg-amber-50 text-amber-600' },
                      { time: '14:00', patient: 'Aoife Ryan', type: 'Monthly prescription', status: 'Confirmed', sc: 'bg-emerald-50 text-emerald-600' },
                    ].map(a => (
              ...
              <motion.div animate={{ y: [0, -8, 0] }} transition={{ duration: 4, repeat: Infinity, ease: 'easeInOut' }}
                ...
                <div className="w-6 h-6 rounded-full bg-purple-500/15 flex items-center justify-center"><Pill className="w-3 h-3 text-pink-600" /></div>
                <span className="text-xs font-semibold text-surface">89% orders handled by AI</span>
              </motion.div>
              <motion.div animate={{ y: [0, -6, 0] }} transition={{ duration: 5, repeat: Infinity, ease: 'easeInOut', delay: 1.5 }}
                ...
                <span className="text-xs font-semibold text-surface">60% fewer phone calls</span>
              </motion.div>
            </motion.div>
          </div>
        </div>
      ...
      <section className="py-12 px-6">
        <div className="max-w-5xl mx-auto grid grid-cols-2 md:grid-cols-4 gap-6">
          {[{ v: '89%', l: 'Fewer no-shows' }, { v: '60%', l: 'fewer phone calls enquiries' }, { v: '24/7', l: 'Admin patient support' }, { v: '24/7', l: 'Patient support' }].map((s, i) => (
      ...
      <section className="py-20 px-6 relative overflow-hidden" style={{ background: `linear-gradient(180deg, rgba(123,97,255,0.03), rgba(123,97,255,0.02))` }}>
        ...
        <div className="relative z-10 max-w-5xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-10 relative">
            ...
            {[
              { step: '01', icon: Pill, title: 'Connect your pharmacy', desc: 'We set up ordering channels — phone, WhatsApp, website. Link your patient database and dispensing system.', color: AC, ring: 'rgba(123,97,255,0.1)', lightBg: 'rgba(123,97,255,0.06)' },
              { step: '02', icon: Bot, title: 'AI handles orders', desc: 'Patients call, text, or order online. AI captures prescriptions, processes repeats, and queues everything for you.', color: '#7B61FF', ring: 'rgba(123,97,255,0.1)', lightBg: 'rgba(123,97,255,0.06)' },
              { step: '03', icon: Zap, title: 'You dispense & deliver', desc: 'Staff focus on care, not admin. Patients get instant responses. No-shows plummet.', color: '#2F8FFF', ring: 'rgba(47,143,255,0.1)', lightBg: 'rgba(47,143,255,0.06)' },
            ].map((p, i) => (
      ...
      <section className="py-16 px-6">
        <ScrollReveal direction="none">
          <motion.div className="max-w-4xl mx-auto rounded-3xl relative overflow-hidden py-16 px-8 text-center"
            ...
            {[Sparkles, Pill, Pill, Bot, Zap, Bot, Star].map((Icon, i) => (
