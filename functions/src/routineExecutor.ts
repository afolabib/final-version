...
function cronToMs(label: string): number {
  const l = (label || '').toLowerCase();
  if (l.includes('15 min'))  return 15 * 60 * 1000;
  if (l.includes('hour'))    return 60 * 60 * 1000;
  if (l.includes('day'))     return 24 * 60 * 60 * 1000;
  if (l.includes('monday') || l.includes('weekly')) return 7 * 24 * 60 * 60 * 1000;
  if (l.includes('friday'))  return 7 * 24 * 60 * 60 * 1000;
  if (l.includes('month'))   return 30 * 24 * 60 * 60 * 1000;
  return 24 * 60 * 60 * 1000; // default: daily
}

// ── Scheduled runner — every 15 minutes ──────────────────────────────────────
export const executeRoutines = functions
  ...
  .onRun(async () => {
    ...
    const due = snap.docs.filter(d => {
      const data = d.data();
      if (!data.nextRunAt) return true; // never run
    ...
    for (const routineDoc of due) {
      try {
        ...
        console.error(`[routineExecutor] Routine ${routineDoc.id} failed:`, (e as Error).message);
      }
    }
  });

// ── Manual trigger from dashboard ─────────────────────────────────────────────
export const triggerRoutine = functions
  .runWith({ timeoutSeconds: 120 })
  .https.onCall(async (data: { routineId: string }, _context) => {
    const { routineId } = data;
    if (!routineId) throw new functions.https.HttpsError('invalid-argument', 'routineId required');
    if (!OPENROUTER_API_KEY) throw new functions.https.HttpsError('failed-precondition', 'OPENROUTER_API_KEY not set');

    const snap = await db.collection('routines').doc(routineId).get();
    if (!snap.exists) throw new functions.https.HttpsError('not-found', 'Routine not found');

    return runRoutine(routineId, snap.data()!);
  });

// ── Core execution ────────────────────────────────────────────────────────────
async function runRoutine(routineId: string, routine: any) {
  ...
  const runRef = db.collection('routine_runs').doc();
  await runRef.set({
  ...
  const trigger = steps.find((s: any) => TRIGGER_TYPES_IDS.has(s.type));
  ...
  const actionSteps = steps.filter((s: any) => !TRIGGER_TYPES_IDS.has(s.type));
...
// ── Step executor ─────────────────────────────────────────────────────────────
async function executeStep(step: any, vars: Record<string, string>, companyId: string): Promise<string> {
  const cfg = step.config || {};

  // Interpolate {{var}} placeholders from previous step outputs
  const interpolate = (s: string) => (s || '').replace(/\{\{(\w+)(?:\.\w+)*\}\}/g, (_, k) => vars[k] || '');
  ...
  switch (step.type) {
    ...
    case 'http_request': {
      ...
      if (!url.startsWith('http')) return 'Skipped — no URL';
...
async function runAgentTask(instruction: string, companyId: string): Promise<string> {
  ...
  for (let round = 0; round < MAX_TOOL_ROUNDS; round++) {
    ...
    for (const model of [KIMI, HAIKU]) {
      const res = await fetch(OPENROUTER_URL, {
        ...
        body: JSON.stringify({ model, messages, tools: TOOL_DEFINITIONS, tool_choice: 'auto', max_tokens: 800 }),
      });
    ...
    for (const tc of msg.tool_calls) {
      const { result } = await executeTool(tc.function.name, tc.function.arguments, { companyId, agentId: 'automation' });
