...
export interface RouteContext {
  agentRole?: string;   // 'ceo', 'sales', 'marketing', 'engineer', 'researcher', 'support'
  trigger?: string;     // 'scheduled', 'manual', 'mention', 'chat'
  taskType?: string;    // 'strategy', 'execution', 'content', 'analysis', 'fast'
  messageHint?: string; // first ~200 chars of message for keyword routing
}
...
export const FALLBACK_CHAINS: Record<string, string[]> = {
  [MODELS.KIMI]:    [MODELS.GPT4O,   MODELS.HAIKU],
  [MODELS.MINIMAX]: [MODELS.KIMI,    MODELS.HAIKU],
  [MODELS.SONNET]:  [MODELS.KIMI,    MODELS.HAIKU],
  [MODELS.GPT4O]:   [MODELS.KIMI,    MODELS.HAIKU],
  [MODELS.HAIKU]:   [MODELS.MINIMAX, MODELS.KIMI],
};
 ...
 */
export function routeModel(ctx: RouteContext): string {
  ...
  if (role === 'marketing' || role === 'content' || role === 'brand') {
    if (isContentTask(hint)) return MODELS.SONNET;
    return MODELS.KIMI; // strategy/planning within marketing still goes to Kimi
  }
  ...
  if (role === 'sales') {
    if (isStrategyTask(hint)) return MODELS.KIMI;
    return MODELS.MINIMAX;
  }
  ...
  if (isStrategyTask(hint))  return MODELS.KIMI;
  if (isContentTask(hint))   return MODELS.SONNET;
  if (isFastTask(hint))      return MODELS.MINIMAX;
...
export function getModelChain(primary: string): string[] {
  return [primary, ...(FALLBACK_CHAINS[primary] || [MODELS.HAIKU])];
}

// ── Keyword heuristics ────────────────────────────────────────────────────────

function isStrategyTask(hint: string): boolean {
