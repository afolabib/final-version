...
async function transcribeWithAssemblyAI(audioUrl: string): Promise<WordEvent[]> {
  // Submit job
  ...
  const { id } = await submitRes.json() as any;
...
function buildTimestampedTranscript(words: WordEvent[]): string {
  ...
  const MARKER_INTERVAL_MS = 10_000;
...
async function findClipsWithAI(transcript: string, videoDurationSec: number): Promise<any[]> {
...
Return ONLY the JSON array.`;
  ...
  try {
    ...
    return clips
      .filter((c: any) => typeof c.startSec === 'number' && typeof c.endSec === 'number')
      .map((c: any) => ({
      ...
      .filter((c: any) => c.endSec - c.startSec >= 10)
      .slice(0, 10);
  } catch (e) {
...
function buildCaptions(words: WordEvent[], startMs: number, endMs: number) {
...
async function saveClips(projectId: string, ownerId: string, clips: any[], words: WordEvent[]) {
...
export const transcribeVideo = functions
  ...
  .https.onRequest(async (req, res) => {
    ...
    try {
      await projectRef.update({ status: 'processing', processingStep: 'Uploading to transcription service…', updatedAt: serverTimestamp() });

      // 1. Download from Storage
      const buffer = await downloadFromStorage(storagePath);

      await projectRef.update({ processingStep: 'Transcribing audio…', updatedAt: serverTimestamp() });
      ...
      if (words.length < 20) {
        ...
        await projectRef.update({ status: 'error', processingError: 'No speech detected in video', updatedAt: serverTimestamp() });
      ...
      await projectRef.update({ status: 'error', processingError: e.message, updatedAt: serverTimestamp() }).catch(() => {});
