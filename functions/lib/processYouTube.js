...
exports.processYouTubeVideo = void 0;
...
function buildTimestampedTranscript(words) {
    ...
    const MARKER_INTERVAL_MS = 10000; // new timestamp every ~10s
...
function extractWordsInWindow(words, startMs, endMs) {
    return words.filter(w => w.tOffsetMs >= startMs && w.tOffsetMs < endMs);
}
function buildCaptions(words, clipStartMs) {
    if (words.length === 0)
        return [];
...
function heuristicClips(words, videoDurationSec) {
...
async function saveClips(projectId, ownerId, videoId, clips) {
...
// ── HTTP handler ──────────────────────────────────────────────────────────────
exports.processYouTubeVideo = functions
    ...
    try {
        await projectRef.update({ status: 'processing', processingStep: 'Fetching transcript…', updatedAt: (0, firebase_1.serverTimestamp)() });
        // 1. Fetch transcript
        const words = await fetchTranscript(videoId);
        const hasTranscript = words.length > 30;
        if (!hasTranscript) {
            // No captions — evenly space placeholder clips
            await projectRef.update({ processingStep: 'No transcript — generating highlights…', updatedAt: (0, firebase_1.serverTimestamp)() });
        ...
        await projectRef.update({ processingStep: 'Reading transcript…', updatedAt: (0, firebase_1.serverTimestamp)() });
        ...
        await projectRef.update({ processingStep: 'Building captions…', updatedAt: (0, firebase_1.serverTimestamp)() });
        let builtClips;
        if (aiClips.length >= 3) {
        ...
        await projectRef.update({ status: 'error', processingError: e.message || 'Unknown error', updatedAt: (0, firebase_1.serverTimestamp)() }).catch(() => { });
